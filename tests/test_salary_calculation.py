...
class TestSalaryCalculation:
    ...
    def test_calculate_salary_missing_gross_salary(self, client):
        """Test salary calculation without gross_salary query param"""
        response = client.post('/api/employees', json={
            'full_name': 'Test',
            'job_title': 'Engineer',
            'country': 'India',
        ...
        response = client.get(f'/api/salary/calculate/{employee_id}')
    ...
    def test_calculate_salary_various_amounts(self, client):
        ...
        for gross in test_cases:
            response = client.get(f'/api/salary/calculate/{employee_id}?gross_salary={gross}')
            assert response.status_code == 200
            data = response.json()
    ...
    def test_calculate_salary_negative_gross(self, client):
        ...
        response = client.get(f'/api/salary/calculate/{employee_id}?gross_salary=-10000')
        assert response.status_code == 422
    ...
    def test_calculate_salary_zero_gross(self, client):
        ...
        response = client.get(f'/api/salary/calculate/{employee_id}?gross_salary=0')
        assert response.status_code == 422

    def test_calculate_salary_invalid_id_type(self, client):
        """Test that a non-integer employee ID returns 422"""
        response = client.get('/api/salary/calculate/abc?gross_salary=50000')
        assert response.status_code == 422
